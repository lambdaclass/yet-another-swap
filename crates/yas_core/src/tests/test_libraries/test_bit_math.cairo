mod BitMathTests {
    mod MostSignificantBit {
        ...
        fn msb_bigger_number() {
            ...
            assert(most_significant_bit(1000000).expect('msb_failed_1000000') == 19, 'msb should be 19');
        }

        #[test]
        fn msb_maximum_256() {
            assert(most_significant_bit(BoundedInt::max()).expect('msb_failed_BoundedInt') == 255, 'msb should be 255');
        }

        #[test]
        fn msb_random_number() {
            // 11000000111001
            assert(most_significant_bit(12345).expect('msb_failed_12345') == 13, 'msb should be 13');
        }
    ...
    mod LeastSignificantBit {
        ...
        fn lsb_bigger_number() {
            // 11110100001001000000
            assert(least_significant_bit(1000000).expect('lsb_failed_1000000') == 6, 'lsb should be 6');
        }

        #[test]
        fn lsb_maximum_256() {
            // 
            assert(least_significant_bit(BoundedInt::max()).expect('lsb_failed_BoundedInt') == 0, 'lsb should be 0');
        }
