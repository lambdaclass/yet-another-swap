mod YASPoolTests {
    ...
    mod Swap {
        ...
        mod PoolCase12 {
            use super::test_pool;
            use yas_core::tests::utils::pool_12::{
                SWAP_CASES_POOL_12, SWAP_EXPECTED_RESULTS_POOL_12
            };
            use yas_core::tests::utils::swap_cases::SwapTestHelper::{POOL_CASES};
            ...
            #[test]
            #[available_gas(200000000000)]
            fn test_pool_12_success_cases() {
                let pool_case = POOL_CASES()[12];
                let expected_cases = SWAP_EXPECTED_RESULTS_POOL_12();
                let (success_swap_cases, _) = SWAP_CASES_POOL_12();
                test_pool(pool_case, expected_cases, success_swap_cases, PRESICION);
            }
            ...
            #[available_gas(200000000000)]
            #[should_panic(expected: ('SPL', 'ENTRYPOINT_FAILED', 'ENTRYPOINT_FAILED'))]
            fn test_pool_12_panics_0() {
                let PANIC_CASE = 0;
                let pool_case = POOL_CASES()[12];
                let (success_swap_cases, panic_swap_cases) = SWAP_CASES_POOL_12();
                let expected_cases =
                    SWAP_EXPECTED_RESULTS_POOL_12(); //get random case, is never executed
                test_pool(
                    pool_case,
                    array![*expected_cases[PANIC_CASE]],
                    array![*panic_swap_cases[PANIC_CASE]],
