mod TestSqrtPriceMath {
    ...
    mod GetNextSqrtPriceFromInput {
        ...
        fn test_amount_in_gt_uint_96_and_zero_for_one_true() {
            ...
            // price mag = 79228162514264337593543950336
            // amount 1267650600228229401496703205376
            // product = amount * price.mag = 295147905179352825856
            // product should be = 100433627766186892221372630771322662657637687111424552206336 (wolf)
            // max u256: 115792089237316195423570985008687907853269984665640564039457584007913129639935 (2**256)-1
            // product / amount = 79228162514264337593543950336 (cairo result)
    ...
    fn encode_price_sqrt(reserve1: u256, reserve0: u256) -> FixedType {
        // let numerator = reserve1 * pow(2, 192);
        // let ratioX92 = reserve1 / reserve0;

        // FP64x96Impl::from_felt(u256_sqrt(ratioX92).into)()
        // FP64x96Impl::from_felt(u256_sqrt(reserve1 / reserve0).into())
        (FP64x96Impl::new(reserve1, false) / FP64x96Impl::new(reserve0, false)).sqrt().round()
    }
